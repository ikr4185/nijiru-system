...
AND del_flg = 0',
	...
	public function updateNotified( $url, $is_notified )
	{
		return $this->execUpdate(
			'UPDATE low_votes_log SET is_notified = ? WHERE url = ?',
			array($is_notified, $url)
		);
	}
	...
	{
		return $this->execUpdate(
			'UPDATE low_votes_log SET del_flg = ? WHERE url = ?',
			array($del_flg, $url)
		);
	}
	...
	{
		return $this->execUpdate(
			"UPDATE low_votes_log SET {$name} = ? WHERE url = ?",
			array($var, $url)
		);
	}
 ...
 ?, ?, ?, ?, ?, ?, ?
		...
		$param = array( $name, $url, 0, 0, $post_date->format('Y-m-d H:i:s'), $add_date->format('Y-m-d H:i:s'), $del_date->format('Y-m-d H:i:s') );
//        var_dump($param);
	 ...
	 * @return bool
	 */
	public function updateLowVotes( $url, $del_date )
	{
		return $this->execUpdate(
			'UPDATE low_votes_log SET del_date = ?, fall_date = now() WHERE url = ?',
			array($del_date, $url)
		);
	}
