...
class RandScpLogic extends AbstractLogic {
	...
	public function getRandScp() {
		...
		$url = "http://scpjapan.wiki.fc2.com/wiki/SCP-".$this->$linkFunc();

		// 未翻訳排除　----------------------------

		// TODO Scraping::run を使用する？
//		$contents = file_get_contents($url, false, $this->context, 1300, 1500);
		$contents = Scraping::run( $url, "1300-1500" );

		mb_regex_encoding("UTF-8");
		if (preg_match("/(ページ名)(.*?)(が見つかりませんでした。)/u",$contents)) {
			return "";
		}
		return $url;
	}
	...
	public function getRandJp() {
		...
		// 番台のランダム取得
		$linkFunc = 'link0';
//		$linkFunc = 'link'. mt_rand(0,1); // todo シリーズⅡに対応
