...
class LoginLogic extends AbstractLogic {
	 ...
	 * @param $point
	 */
	public function setWelcomeMsg($user_name,$point) {
		$this->setMsg("Welcome User {$user_name} @ {$point} nijiru points");		
	}
	 ...
	 * @return bool
	 */
	public function checkLoginBonus($id) {
		
		// 最終ログイン日時取得
		$lastLoginArray = $this->Users->getLastLogin($id);

		if (!$lastLoginArray){
			$this->setMsg("最終ログイン日時の読み込みに失敗しました");
			return false;
		}
		$lastLogin = $lastLoginArray["last_login"];
	...
	protected function registerValidate($id,$pass,$checkPass,$name) {
		...
		if (empty($id)) {
			...
			$this->setMsg("パスワードは半角英数字4文字以上で入力してください");
			return false;
		} else if ((empty($checkPass)) || ($pass != $checkPass)) {
