...
class DownloadLogic extends AbstractLogic
{
    ...
    public function consumePoint($userId, $downloadId)
    {
        ...
        $userNumber = $this->Users->getNumberById($userId);
        $havingPoint =  $this->NjrAsset->getAssets($userNumber);
        
        if ($price > $havingPoint) {
            return false;
        }
        return $this->NjrAsset->delPoint($price, $userNumber);
    }
    ...
    public function downloadMethod($fileId)
    {
        ...
        // ファイル存在チェック
        if (!$this->isExist($fileDir . $data["contents_path"])) {
            return;
        }
        
        // オープンできるか確認
        if (!$fp = $this->isOpenable($fileDir . $data["contents_path"])) {
            return;
        }
        fclose($fp);
        
        // ファイルサイズの確認
        if (!$content_length = $this->fileSizeCheck($fileDir . $data["contents_path"])) {
            return;
        }
        ...
        /* ファイルを読んで出力 */
        if (!$this->readFile($fileDir . $data["contents_path"])) {
            return;
        }
    }
