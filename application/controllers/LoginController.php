...
class LoginController extends AbstractController
{
    ...
    {
        // 入力値検証
        if (!$idInfo = $this->checkPost($id, $pass)) {
            return;
        }
        ...
        // ログインボーナスの計算と検証
        if (!$this->logic->setLoginBonus($id)) {
            return;
        }

        // ニジポ取得
        $asset = $this->logic->getAssets($id);

        // ニジポをセッションに格納
        $this->input->setSession("point", $asset["point"]);
        $this->input->setSession("tera_point", $asset["tera_point"]);

        // Welcomeメッセージ生成
        if ($is_welcome) {
            $this->logic->setWelcomeMsg($idInfo["user_name"], $asset);
        }
    ...
    {
        // バリデートチェック
        if (!$this->logic->validate($id, $pass)) {
            return false;
        }

        // ユーザー情報の取得
        if (!$idInfo = $this->logic->getIdInfo($id)) {
            return false;
        }

        // パスワード照合
        if (!$this->logic->checkPass($pass, $idInfo["password"])) {
            return false;
        }
    ...
    protected function delUserSession()
    {
        ...
        $this->input->delSession("point");
        $this->input->delSession("tera_point");
    }
